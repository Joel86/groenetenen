...
class JdbcFiliaalRepository implements FiliaalRepository {
	...
	private static final String SQL_FIND_AANTAL_WERKNEMERS = "select count(*) from " + 
			"werknemers where filiaalId = ?";
	private static final String SQL_DELETE = "delete from filialen where id = ?";
	private static final String SQL_UPDATE = "update filialen set naam=?, hoofdFiliaal=?, " + 
			"straat=?, huisNr=?, postcode=?, gemeente=?, inGebruikName=?, waardeGebouw=? " + 
			"where id=?";
	...
	public void update(Filiaal filiaal) {
		jdbcTemplate.update(SQL_UPDATE, filiaal.getNaam(), filiaal.isHoofdFiliaal(), 
				...
				Date.valueOf(filiaal.getInGebruikName()), filiaal.getWaardeGebouw(), 
				filiaal.getId());
	}
	@Override
